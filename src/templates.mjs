...
<body>
  ...
  ${body}
  <footer class="site-footer"><div class="shell footer"><span>Lorenzo Pulcini — Financial theory × software</span><span>St Louis, MO · 2026 · <a href="/admin">Admin</a></span></div></footer>
</body>
</html>`;
}
...
function cardMedia(data, article) {
  ...
  return media ? `<div class="card-media"><img src="/media/${media.id}" alt="${e(media.alt || "")}" loading="lazy"></div>` : "";
}
...
export function articlePage(data, article) {
  ...
  const body = (article.body || []).map((paragraph, index) => `${articleBlock(data, paragraph)}${custom?.placement === `after-${index + 1}` ? articleEmbed(article) : ""}`).join("");
  const embedded = custom?.placement === "before" ? `${articleEmbed(article)}${body}` : custom?.placement === "end" ? `${body}${articleEmbed(article)}` : body;
